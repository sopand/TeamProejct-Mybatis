...
public class ProductController {
	 ...
	 * 
	 * @param prin  = 스프링 시큐리티의 로그인 정보를 가져올 수 있는 객체로, 현재 제품을 등록하려는 사용자의 아이디값을 가져오기
	 *              위함.
	 ...
	 * @param reDirect       = forward방식으로 view를 반환한다면 새로고침을 할 경우 중복되는 데이터가 입력될 위험성이
	 *                       존재하기 떄문에 Redirect를 하는데, Redirect시 Option에 등록에 제품의
	 *                       고유번호(PK)가 필요하기 때문에 그값을 View로 보내주기 위해 사용되는 객체,
	 ...
	 * @param model            = View에 리턴시켜줄 데이터를 설정해주기 위한 객체,
	 * @param r_pnickname_m_fk = 해당 제품의 제품 명을 가져온다 ( Review 관련 데이터를 얻어오기 위함 ) < -
	 *                         Review 파트담당하는 팀원의 파라미터값,
	...
	@GetMapping("/options/chk") // Option페이지로 이동시 해당 제품에 대한 기존 옵션 존재유무를 판단 , 단일 ,다중옵션의 중복을 배제하기 위해 반대되는 옵션페이지를
								// 지우기 위한 맵핑.
	...
	/**
	 * 제품 생성후 옵션생성 페이지에서 옵션에 대한 생성을 1개도 하지 않고 페이지 이탈을 할 경우에 제품을 삭제 처리시키는 로직입니다. 제품이
	 * 무조건 1개이상의 옵션을 가지게 강제하기 위한 로직
	 * 
	 * @param opt_pid_p_fk = 옵션을 삭제 하려 하는 제품의 고유 번호 (PK) 입니다.
	 * @return
	 */
	@ResponseBody
	...
	/**
	 * 옵션 생성페이지로 이동시켜주는 용도의 맵핑입니다
	 * 
	 * @param model = Option을 생성할때 기본적으로 제품의 고유번호를 받아와 FK로 설정하기 때문에 p_id를 View로 보내기
	 *              위한 객체입니다.
	 * @param p_id  = 제품의 고유번호 입니다
	 * @return
	 */
	@GetMapping("/options")
	...
	/**
	 * 옵션을 생성하기 위한 기능을 담당하는 맵핑입니다. ResponseBody와 AJax를 이용하여 비동기 통신으로 작동하게 설정했습니다
	 * 
	 * @param optionRequest = 사용자가 입력한 옵션에 대한 정보가 들어있는 객체
	 * @param opt_pid_p_fk  = 옵션을 등록하려고 하는 제품의 고유번호입니다.
	 * @return
	 */
	@ResponseBody
	@PostMapping("/options")
